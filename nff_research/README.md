...
This directory freezes the local research source used for the NFF v2.3
...
open-to-open remains diagnostic. The v2.3 pass adds a turnover-controlled
contrarian diagnostic, five paired Hawkes eligibility gates, bipower-adjusted
jump-tail diagnostics, and a conditional OOF baseline for the mechanical
execution-cost proxy.

Known limits:

- The runner filename is retained for compatibility; contracts and reports
  explicitly identify research version 2.3 and base runner version 2.1.
...
- The portfolio output is a transparent sleeve-accounting proxy, not a full
  Qlib Recorder strategy backtest. It does not model price drift, borrow,
  spreads, impact, partial fills, or account-level sleeve netting.
