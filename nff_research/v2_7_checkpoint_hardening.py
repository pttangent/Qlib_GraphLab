...
def _upstream_fingerprint(campaign: Any, trade_date: str) -> str:
    ...
    for root in roots:
        if not root.exists():
            continue
        for schema_root in sorted(root.glob("schema=*")) if root.name != "nvg_supplement" else []:
            partition = schema_root / f"date={trade_date}"
            for path in sorted(partition.rglob("*.parquet")) if partition.exists() else []:
                stat = path.stat()
                records.append(
                    {
        ...
        if root.name == "nvg_supplement":
            for dataset_root in sorted(root.iterdir()):
                if not dataset_root.is_dir():
                    continue
                for schema_root in sorted(dataset_root.glob("schema=*")):
                    partition = schema_root / f"date={trade_date}"
                    for path in sorted(partition.rglob("*.parquet")) if partition.exists() else []:
                        stat = path.stat()
                        records.append(
...
def install(campaign: Any) -> None:
    ...
    def factor_contract(group: pd.DataFrame, frame: pd.DataFrame) -> str:
        base = base_factor_contract(group, frame)
        ...
        for universe in ("liquid_common_adv20_top1000", "common_structural"):
            for label_column in (
                ...
                trades = features.loc[base_mask].get(
                    ...
                    pd.Series(np.nan, index=base_label.index),
                )
                base_metadata = {
                ...
                for variant in ("raw", "winsorized", "neutralized"):
                    if variant == "neutralized":
                        ...
                        if cached is None:
                            continue
                        signal_source = cached[0].reindex(base_label.index)
                        target = cached[1].reindex(base_label.index)
                        available = [column for column in factor_columns if column in signal_source]
                    else:
                    ...
                    for block_id, start in enumerate(
                        range(0, len(available), max(1, context.factor_block_size))
                    ):
                        selected = available[
                            start : start + max(1, context.factor_block_size)
                        ]
                        if not selected:
                            continue
                        ...
                        if variant == "winsorized":
                            ...
                            grouped = signals.groupby(
                                groups, sort=False, group_keys=False
                            )
                            signals = signals.clip(
                                lower=grouped.transform("quantile", q=0.01),
                                upper=grouped.transform("quantile", q=0.99),
                            )
                        metadata = {**base_metadata, "variant": variant}
                        rows: list[dict[str, Any]] = []
                        with ThreadPoolExecutor(
                            max_workers=max(1, context.intra_workers)
                        ) as executor:
                            futures = [
                                executor.submit(
