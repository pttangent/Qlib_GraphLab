...
def _regular_session_filter(df: pd.DataFrame, column: str) -> pd.Series:
    ...
    return (minute >= 14 * 60 + 30) & (minute <= 21 * 60)
...
def _residualize_matrix(values: pd.DataFrame, controls: pd.DataFrame, min_n: int = 40) -> pd.DataFrame:
    ...
    for dt, idx in values.groupby(level="datetime", sort=False).groups.items():
        ...
        v = values.loc[idx]
        base = c.replace([np.inf, -np.inf], np.nan).notna().all(axis=1) & v.replace([np.inf, -np.inf], np.nan).notna().all(axis=1)
        if int(base.sum()) < min_n:
            continue
        x = c.loc[base].to_numpy(dtype="float64")
        x = np.column_stack([np.ones(x.shape[0]), x])
        y = v.loc[base].to_numpy(dtype="float64")
        try:
            coef, *_ = np.linalg.lstsq(x, y, rcond=None)
        except np.linalg.LinAlgError:
            continue
        result.loc[v.loc[base].index, values.columns] = (y - x @ coef).astype("float32")
    return result
...
def staggered_portfolio_proxy(
    ...
    for label_column in [c for c in labels.columns if c.startswith("return_open_to_open__h")]:
        ...
        for feature in portfolio_features:
            ...
            for dt, block in work[[feature, "label"]].dropna().groupby(level="datetime", sort=False):
                if dt.minute % 15 != 0 or len(block) < min_n:
                    continue
...
def run_date(trade_date: str, horizons: list[int], out_root: Path, controls_path: Path, min_n: int) -> dict[str, Any]:
    ...
    portfolio = staggered_portfolio_proxy(features, labels, label_masks, controls, trade_date, min_n)
    ...
    meta = {
        ...
        "universe_contract": {
            "own_feature_universe": "all rows loaded by the inner NFF/Qlib decision-time join, with per-feature non-null coverage applied only inside each IC.",
...
def _block_bootstrap_mean(values: pd.Series, reps: int = 300, block_size: int = 5) -> dict[str, float]:
    ...
    n = x.size
    if n < block_size:
        return {"bootstrap_mean": math.nan, "bootstrap_ci_low": math.nan, "bootstrap_ci_high": math.nan, "bootstrap_p_two_sided": math.nan}
...
def write_research_contract(out_root: Path) -> None:
    contract = {
        "name": "NFF v2.1 neutralized representative validity and staggered portfolio proxy",
        "rank_ic_methods": [
            "minute_mean_cs_rank_ic",
            "pooled_cs_rank_ic",
            "minute_mean_cs_rank_ic_residualized",
        ...
        "universe_contract": {
            "own_feature_universe": "all rows loaded by the inner NFF/Qlib decision-time join.",
        ...
        "portfolio_proxy_contract": {
            ...
            "capital": "One unit gross exposure per cohort; results are cohort return observations, with turnover and simple cost columns.",
        ...
        "known_limits": [
            ...
            "Portfolio output is a transparent staggered proxy, not a full Qlib Recorder/model backtest.",
...
def aggregate(out_root: Path) -> None:
    ...
    report = [
        ...
        "- Portfolio proxy: 15-minute staggered top/bottom decile long-short cohorts with turnover and simple cost columns.",
...
def main() -> int:
    ...
    if args.worker:
        ...
        print(json.dumps(run_date(args.worker_date, args.horizons, out_root, controls_path, args.min_cs_n), indent=2, default=str))
        return 0
