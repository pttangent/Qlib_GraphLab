        ...
        if available_gb >= min_available_gb:
            for slot in range(slots):
                ...
                try:
                    ...
                    os.close(fd)
                _ACTIVE_LEASE = _Lease(path=path, slot=slot, acquired_at=time.perf_counter(), trade_date=trade_date)
                _emit(
                    P,
...
def _manifest_is_reusable(C: Any, manifest: Mapping[str, Any], root: Path, contract: str, rows: int) -> bool:
...
def _streaming_derive_all(P: Any, frame: pd.DataFrame, prototypes: list[dict[str, Any]]):
    C = P.C
    if C.CTX is None:
        return _ORIGINALS["derive_all"](frame, prototypes)
    specs = P.V26.SPEC_REGISTRY[P.V26.SPEC_REGISTRY["family"].isin(list("ABCDEFGHIJK"))]
    ...
    for (family, window), group in specs.groupby(["family", "window"], sort=False):
        ...
        if manifest_path.exists():
            ...
            if _manifest_is_reusable(C, manifest, root, contract, len(frame)):
                ...
                C._event("factor_block", "reused", family=family, window=window, streaming=True)
                continue
        ...
        for block_id, start in enumerate(range(0, len(rows), C.CTX.factor_block_size)):
            selected_rows = rows[start : start + C.CTX.factor_block_size]
            ...
            if generated:
                ...
                entries.append({"path": path.name, "columns": list(generated), "factors": status})
                del part
            del generated
        C._atomic_json(
            manifest_path,
        ...
        C._event("factor_block", "complete", family=family, window=window, factors=len(runtime), streaming=True)
        del raw, values, rows, entries
...
def _manifest_factor_status(P: Any, trade_date: str) -> dict[str, dict[str, Any]]:
    ...
    for manifest_path in P._factor_manifests(P._CONFIG, trade_date):
        ...
        for block in manifest.get("blocks", []):
            for name in block.get("columns", []):
                ...
                if not status:
                    status = {"status": "SUCCESS", "non_null_rate": math.nan}
                result[str(name)] = status
    return result


def _streaming_feature_registry(P: Any, features: pd.DataFrame, trade_date: str, expected: set[str]) -> pd.DataFrame:
    ...
    for feature in sorted(expected):
        ...
        non_null_count = int(round(rate * len(features))) if math.isfinite(rate) else 0
        row = {
            "trade_date": trade_date,
            "feature": feature,
...
def _optimized_materialize_date(P: Any, config: dict[str, Any], trade_date: str) -> dict[str, Any]:
    P.bootstrap(config)
    C = P.C
    ...
    C.write_schema_audit(P.R.WAREHOUSE_ROOT, C.CTX.root / "schema", trade_date)
    C._event("pipeline_materialize", "running", execution_mode="memory_streaming_peak_gated")
    try:
        ...
        controls = P.R.join_daily_controls(features, trade_date, P._controls_path(config))
        universes = pd.DataFrame(P.R.universe_masks(features, controls), index=features.index)

        expected = set(P.V26.FULL_FACTOR_NAMES)
        inventory = P._factor_block_inventory(config, trade_date)
        present = set(inventory.get("feature", ())) if not inventory.empty else set()
        ...
        registry = _streaming_feature_registry(P, features, trade_date, expected)

        P._atomic_parquet(support, stage_root / "support.parquet")
        P._atomic_parquet(labels, stage_root / "labels.parquet")
        P._atomic_parquet(pd.DataFrame(label_masks, index=features.index), stage_root / "label_masks.parquet")
        P._atomic_parquet(controls, stage_root / "controls.parquet")
        P._atomic_parquet(universes.astype("boolean"), stage_root / "universes.parquet")
        P._atomic_parquet(registry, stage_root / "feature_registry.parquet", index=False)
        P._atomic_parquet(inventory, stage_root / "factor_block_inventory.parquet", index=False)
        ...
        meta = {
            ...
            "peak_phase_gate": {
                ...
                "peak_entry_min_available_gb": float(settings.get("peak_entry_min_available_gb", 48.0)),
            },
...
def _install_after_bootstrap(P: Any, config: dict[str, Any]) -> None:
    ...
    def gated_loader_load(self: Any, *args: Any, **kwargs: Any):
        trade_date = P.C.CTX.trade_date if P.C.CTX is not None else "unknown"
        _acquire_peak_lease(P, config, trade_date)
        _emit(P, "materialize_peak_phase", "warehouse_load_running", trade_date=trade_date)
        try:
            return _ORIGINALS["loader_load"](self, *args, **kwargs)
        finally:
            _emit(P, "materialize_peak_phase", "warehouse_load_complete", trade_date=trade_date)
    ...
    def release_after_venue(*args: Any, **kwargs: Any):
        ...
        try:
            ...
            _emit(P, "materialize_peak_phase", "wide_merge_complete", trade_date=trade_date)

    loader_class.load = gated_loader_load
    P.C.FF.merge_venue_aggregates = release_after_venue
    P.C.FF.derive_all = lambda frame, prototypes: _streaming_derive_all(P, frame, prototypes)
...
def install(P: Any) -> None:
    ...
    P.materialize_date = lambda config, trade_date: _optimized_materialize_date(P, config, trade_date)
